...
static void snd_soc_instantiate_card(struct snd_soc_card *card)
{
	...
	/*
		首先判断platform_list 中是否有platform,
	进行遍历

		platfrom 再s3c-dma.c 中s3c24xx_soc_platform_init 定义的

	 并且 platfrom =  s3c24xx_soc_platform
	*/
	found = 0;
	...
	/*   
	遍历dai_list ；
        注册结构体struct snd_soc_dai  是在wm8796.c 这个文件
        中
	*/
	ac97 = 0;
	for (i = 0; i < card->num_links; i++) {
	...
	/*   	
		如果相应的card->dai_link[i].codec_dai->ops 为空,
	则赋值为null_dai_ops
	*/
	for (i = 0; i < card->num_links; i++) {
	...
	/*   
		调用card  的probe 函数 ，card =  s3c6410 
	s3c6410 的probe 为NULL
	*/
	if (card->probe) {
		ret = card->probe(pdev);
		if (ret < 0)
	...
	/*
		这个相应的也再s3c6410_snd_devdata 中有相应的
	定义
		s3c6410_snd_devdata->s3c6410->s3c6410_dai->wm8976_dai

		通过分析: cpu_dai->probe 函数就是wm8976_dai中
	的probe 函数，故为NULL
	*/
	for (i = 0; i < card->num_links; i++) {
		struct snd_soc_dai *cpu_dai = card->dai_link[i].cpu_dai;
	...
	/*	
	这个相应的也再s3c6410_snd_devdata 中有相应的
	定义, 就是该结构体下的codec_dev成员

	 codec_dev = soc_codec_dev_wm8976

	 故codec_dev->probe 函数就是wm8976_probe
	...
	*/
	...
	/*
	  s3c6410_snd_devdata里面设置了card
           card == s3c6410 
           这个再平台设备注册(s3c6410_init )的时候定义的
	*/
	for (i = 0; i < card->num_links; i++) {
	...
	/*
		wm8976_probe -> wm8976_add_i2c_device -> wm8976_i2c_driver ->
		wm8976_i2c_probe -> wm8976_init -> snd_soc_new_pcms 
		-> snd_card_create
	*/
...
static int soc_probe(struct platform_device *pdev)
{
	...
	/*
		获取snd_soc_device 设备 
	对应再s3c6410_init 中设置的；

	故socdev = s3c6410_snd_devdata
	*/
	struct snd_soc_device *socdev = platform_get_drvdata(pdev);
	struct snd_soc_card *card = socdev->card;
...
static int snd_soc_register_card(struct snd_soc_card *card)
{
	...
	/*
		将card->list 加入到card_list ，以后会遍历card_list 
	进行匹配
	*/
...
static int __init snd_soc_init(void)
{
	...
	/*
		这个注册和s3c6410_init 进行匹配, 从而调用
	soc_driver 中的probe函数；
	*/
