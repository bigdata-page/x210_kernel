...
/*
ASoC被分为Machine、 Platform和Codec三大部分，其中的Machine驱动
负责  platform 和Codec之间的耦合以及部分和设备或板子特定
的代码，再次引用上一节的内容： Machine驱动负责处理机
器特有的一些控件和音频事件（例如，当播放音频时，
需要先行打开一个放大器）；单独的Platform和Codec驱动是不
能工作的，它必须由Machine驱动把它们结合在一起才能完
成整个设备的音频处理工作。	

ASoc 的一切都从Machine驱动开始，包括声卡的注册，绑定
Platform 和Codec驱动等等。
*/
