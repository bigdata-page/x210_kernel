...
static int ehci_hcd_s5pv210_drv_probe(struct platform_device *pdev)
{
	...
	/*  
		平台设备文件在devs.c 中定义
		pdev->resource == s3c_usb__ehci_resource

		故pdev->resource[1].flags == ORESOURCE_IRQ
	*/
...
/* 对应的是 struct platform_device s3c_device_usb_ehci */
