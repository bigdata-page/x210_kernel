		...
		struct device *dev, const char *bus_name)
{
	...
	/*  
		将hcd 赋值给dev->p-> driver_data,
	后面肯定会用到,一般情况是从设备中获取hcd
	*/
	dev_set_drvdata(dev, hcd);
	kref_init(&hcd->kref); /*  将hcd->kref 赋值为1 */
		...
		unsigned int irqnum, unsigned long irqflags)
{
	...
	/* 
		注册usb 总线，hcd->self 在前面创建hcd 的时候
	初始化了usb_create_hcd(在ehci_hcd_s5pv210_drv_probe中调用)
	*/
