...
{
	printk(KERN_ALERT "the location is %d line of %s\n",__LINE__, __FUNCTION__);
	struct s3cfb_global *fbdev =
		platform_get_drvdata(to_platform_device(fb->device));
	struct s3c_platform_fb *pdata = to_fb_plat(fbdev->dev);
	struct s3cfb_window *win = fb->par;
...
static void s3cfb_init_fbinfo(struct s3cfb_global *ctrl, int id)
{
	...
	/*  
		这个 s3cfb_ops   在struct file_operations fb_fops 中的open 函数中
	调用
	*/
	fb->fbops = &s3cfb_ops;
...
static int s3cfb_alloc_framebuffer(struct s3cfb_global *ctrl)
{
	...
	/*   
		nr_wins = 5  ;  在mach-x210.c中
			static struct s3c_platform_fb ek070tn93_fb_data  中定义的 
	*/
	for (i = 0; i < pdata->nr_wins; i++) {
		/* 
			这个函数具体就是将crtl->dev 赋值给 info->device ；
		而在s3cfb_probe函数中fbdev->dev = &pdev->dev; 也就是说
		ctrl->dev 就是平台设备的成员dev
		*/
		...
		/*	
			初始化struct fb_info 这个结构体,后面如果
		用到fb_info 可以在这里查看相应的设置。

		其中设置了fb_info -> fbops, 在字符设备open/read/write
		等等函数中都有用到
	...
	*/
	for (i = pdata->default_win;
		i < pdata->nr_wins + pdata->default_win; i++) {
			/*
				这里注册五个fb  , 对应文件系统
			/sys/class/graphics 中的fb0/1/2/3/4
			*/
			j = i % pdata->nr_wins;
...
/*
	struct platform_device  s3c_device_fb   通过这就知道
在devs.c 中定义了平台设备
*/
...
static int __devinit s3cfb_probe(struct platform_device *pdev)
{
	...
	/* 	
		s3cfb_set_platdata(&ek070tn93_fb_data); 和下面这些功能相反
	故 pdata = ek070tn93_fb_data    在mach-x210.c中有定义
	*/
	...
	/* 
	这个pdata->lcd 数据是ek070tn93_fb_data 中定义的，具体见这个
	结构体

	lcd的参数在下面的s3cfb_init_global 中初始化用到了
	*/
	...
	*/
	/*
		这些是硬件相关的操作
		pdata->cfg_gpio  对应于ek070tn93_cfg_gpio
		pdata->clk_on 对应于为NULL
	*/
	if (pdata->cfg_gpio)
		pdata->cfg_gpio(pdev); /*只是设置引脚为LCD_VD模式，然后赋值为1*/

	if (pdata->clk_on)/*  pdata->clk_on == NULL  */
	...
	*/
	...
	/*
		下面的涉及到是底层和硬件相关的操作.
	这类和应该相关的函数都在 s3cfb_fimd6x.c 中定义
	*/
	...
	/*  
		s3cfb_alloc_framebuffer  中s3cfb_init_fbinfo 函数里面
		进行了一系列初始化

		主要是对fbdev-> fb[i] 就是struct fb_info 这个
		结构体进行初始化
	*/
