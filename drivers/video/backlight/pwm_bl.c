...
/*
	通过分析可知 pdev == smdk_backlight_device
	从而data = smdk_backlight_data
...
static int pwm_backlight_probe(struct platform_device *pdev)
{
	...
	*/
	...
	/*
		这里面主要是注册了创建了，
		然后将b1->ops = &pwm_backlight_ops
				   b1->props = props	
		并将pb 赋值给了b1->dev-> p->driver_data
	*/
	...
	/* 关于proprs 设置了max_brightness==255 和dft_brightness==255 */
	bl->props.brightness = data->dft_brightness;

	/*  就是调用pwm_backlight_update_status   */
	backlight_update_status(bl);

	/* 将pdev->dev->p->driver_data = b1 */
